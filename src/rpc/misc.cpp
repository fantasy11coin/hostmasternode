...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total betfint balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized betfint balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "betfint|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use '+' to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "betfint")
            + HelpExampleRpc("debug", "betfint+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given betfint address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The betfint address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The betfint address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Betfint address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are betfint addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) betfint address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The betfint address to use for the signature.\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Betfint features */
    { "betfint",               "mnsync",                 &mnsync,                 true,  {} },
    { "betfint",               "spork",                  &spork,                  true,  {"value"} },
