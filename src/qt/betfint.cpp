...
{
    return QCoreApplication::translate("betfint-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in betfint.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in betfint.qrc)
...
/** Class encapsulating Betfint Core startup and shutdown.
...
/** Main Betfint application object */
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        // betfint: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Betfint Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(betfint);
    Q_INIT_RESOURCE(betfint_locale);
    ...
    /// 6. Determine availability of data directory and parse betfint.conf
    ...
    if(!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("Betfint Core"),
    ...
    // betfint: links repeatedly have their payment requests routed to this process:
